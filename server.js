...
// 🧠 回放紀錄上限：logReplayEvent 原本會無上限一直 push，長時間對局會讓記憶體
//    持續增長。超過這個筆數就把最舊的事件丟掉，只保留最近 N 筆（第二層保險，
//    主因是下面 batch 傳送迴圈的 bug，但這裡也一起補強避免正常對局跑很久累積出問題）。
...
// ── 玩家自訂名稱清理／驗證 ──────────────────────────────
// name 一律由 client 端 JOIN 封包傳入，但 client 可被繞過（改前端程式碼或直接送封包），
// 所以真正的驗證必須在這裡（server 端）做，不能只信任前端已經檢查過。
// 1) 移除控制字元與 HTML 特殊字元，避免這個名稱之後被其他 client 端當 innerHTML 插入時
//    造成 XSS（存進 room 後會廣播給全房間所有人，等於是儲存型 XSS + 自動群發）。
// 2) 限制長度，避免超長字串塞爆大廳版面或封包。
// 3) 簡單黑名單過濾明顯不雅字詞（可依實際使用情況擴充這份清單）。
const PLAYER_NAME_MAX_LEN = 10;
const PLAYER_NAME_BLOCKLIST = ['幹你', '肏你', '3小', '三小', '白痴', '智障', 'fuck', 'shit', 'bitch', 'sb', 'nmsl'];
function sanitizePlayerName(raw, fallback){
  if(typeof raw !== 'string') return fallback;
  const cleaned = raw
    .replace(/[\u0000-\u001F\u007F<>&"'`]/g, '')   // 控制字元 + HTML/屬性注入常用符號
    .trim()
    .slice(0, PLAYER_NAME_MAX_LEN);
  if(!cleaned) return fallback;   // 空字串／全是被濾掉的字元 → 退回預設「玩家N」
  const lower = cleaned.toLowerCase();
  if(PLAYER_NAME_BLOCKLIST.some(w => lower.includes(w.toLowerCase()))) return fallback;
  return cleaned;
}

// ── 陣營（team）驗證：只接受 'RED' / 'BLUE'，其餘一律視為未選定（null）──
// 原本任何欄位都直接信任 client 送來的字串，GAME_START 檢查 20 點編制上限時是用
// teamPtsCheck[p.team] 累加（見下方 GAME_START handler），如果 team 是伺服器沒預期
// 過的怪字串，那筆累加會直接被跳過——等於故意送一個亂七八糟的 team 值，就能讓自己
// 的點數完全不被計入上限檢查，繞過編制點數限制。統一收斂成這個函式後，team 永遠
// 只會是 'RED'、'BLUE' 或 null 三種值，這個漏洞就不成立了。
function sanitizeTeam(raw){
  return (raw === 'RED' || raw === 'BLUE') ? raw : null;
}

// ── 危險物件 key 防護：uid 被拿來直接當 room.units 的屬性名稱使用
//    （例如 currentRoom.units[uid] = {...}），如果 uid 剛好是 '__proto__' 這種
//    JS 內建的特殊屬性名，會讓後續的屬性賦值變成竄改 Object.prototype 本身
//    ——這不是竄改單一房間的資料，而是污染整個 Node process 裡「所有」物件的
//    原型，會讓伺服器上所有房間、所有玩家的資料都出現無法預期的欄位/行為，
//    是比單純的房間資料錯亂嚴重得多的問題。凡是把 client 傳來的字串當成
//    物件 key 使用之前，一律先過這一關。
const _UNSAFE_OBJECT_KEYS = new Set(['__proto__', 'constructor', 'prototype']);
function isSafeUnitID(uid){
  return typeof uid === 'string' && uid.length > 0 && !_UNSAFE_OBJECT_KEYS.has(uid);
}
...
function _syncUnitHPInRoom(room, uid, hp){
  if(!room || !room.units || !isSafeUnitID(uid) || hp == null) return;
  if(hp <= 0){
...
function logReplayEvent(room, type, payload, fromPID){
  ...
  // 超過上限就丟掉最舊的，避免陣列無限成長吃光記憶體
  if(room.replayEvents.length > MAX_REPLAY_EVENTS){
    room.replayEvents.splice(0, room.replayEvents.length - MAX_REPLAY_EVENTS);
  }
}
...
setInterval(() => {
  ...
  console.log(`[Memory] heapUsed=${(mem.heapUsed/1048576).toFixed(1)}MB, rss=${(mem.rss/1048576).toFixed(1)}MB, pendingBatches=${_pendingBatches.size}個連線/共${pendingTotal}筆待送, rooms=${rooms.size}`);
}, 30000);
...
wss.on('connection', function (ws) {
  ...
  const _KNOWN_PACKET_TYPES = new Set([
    ...
    'MAP_INIT','TEAM_LOADOUT_UPDATE','PLAYER_READY','PLAYER_UNREADY','PLAYER_DEPLOYED','MATCH_STATS_REVEAL',
  ...
  const _THROTTLED_PACKET_TYPES = new Set(['INTEL_SYNC']); // 已知但仍有洪水風險，照樣限流
  ...
  ws.on('message', function (raw) {
    ...
    if(needsThrottle){
      ...
      _lastRefill = now;
      if(_msgTokens < 1){
        console.warn(`[RateLimit] 玩家${currentPlayer?.playerID ?? '?'} 封包（${packet.type}）頻率超限，丟棄本次封包`);
        return;
      }
      _msgTokens -= 1;
    }
  ...
  function _handleMessage(packet){
    ...
    if (type === 'JOIN') {
      ...
      if(rejoinToken && room.playersByToken.has(rejoinToken)){
        ...
        if(payload?.team){
          const reqTeam = sanitizeTeam(payload.team);
          if(reqTeam) currentPlayer.team = reqTeam;
          else console.warn(`[Security] 玩家${currentPlayer.playerID} 重連時送出不合法的 team 值（${JSON.stringify(payload.team)}），已忽略`);
        }
      } else {
        ...
        currentPlayer = {
          ...
          team:     wantsSpectator ? null : sanitizeTeam(payload?.team),
          name:     sanitizePlayerName(payload?.name, wantsSpectator ? `觀察者${assignedPID}` : `玩家${assignedPID}`),
      ...
      ws.send(JSON.stringify({
        ...
        payload: {
          ...
          alreadyDeployed:     !!((room.deployedPlayers && room.deployedPlayers[assignedPID]) || (room.spawnedPlayers && room.spawnedPlayers[assignedPID])),
    ...
    switch (type) {
     case 'MAP_INIT':
      ...
      case 'TEAM_LOADOUT_UPDATE': {
        ...
        const requestedTeam = packet.payload?.team;
        const newTeam      = sanitizeTeam(requestedTeam);
        // 不合法的 team 值（不是 'RED'/'BLUE'）直接整包忽略，不更新、不廣播——
        // 這正是原本能被拿來繞過 GAME_START 20 點編制上限檢查的漏洞（見 sanitizeTeam 註解）。
        if(requestedTeam != null && !newTeam){
          console.warn(`[Security] 玩家${currentPlayer.playerID} 送出不合法的 team 值（${JSON.stringify(requestedTeam)}），已忽略此 TEAM_LOADOUT_UPDATE 封包`);
          break;
        }
      ...
      case 'PLAYER_READY': {
        const requestedReadyTeam = packet.payload?.team;
        const readyTeam = sanitizeTeam(requestedReadyTeam);
        if(requestedReadyTeam != null && !readyTeam){
          console.warn(`[Security] 玩家${currentPlayer.playerID} 送出不合法的 team 值（${JSON.stringify(requestedReadyTeam)}），已忽略此 PLAYER_READY 封包`);
          break;
        }
        // 記錄玩家準備狀態與編制
        currentRoom.readyPlayers = currentRoom.readyPlayers || {};
        currentRoom.readyPlayers[currentPlayer.playerID] = packet.payload;
        currentPlayer.team    = readyTeam;
      ...
      // ── 玩家取消準備：遊戲開始前才允許 ──
      case 'PLAYER_UNREADY':
        if(currentRoom.gameStarted) break;   // 遊戲開始後不能取消
        if(currentRoom.readyPlayers) delete currentRoom.readyPlayers[currentPlayer.playerID];
        console.log(`[Ready] 玩家${currentPlayer.playerID} 取消準備，房間共 ${Object.keys(currentRoom.readyPlayers || {}).length} 人準備`);
        logReplayEvent(currentRoom, 'PLAYER_UNREADY', {}, currentPlayer.playerID);
        broadcastAll(currentRoom, {
          type: 'READY_UPDATE',
          roomID,
          payload: {
      ...
      // ── 賽後戰報攤牌：只轉發給其他人，不轉給自己 ──
      case 'MATCH_STATS_REVEAL':
        logReplayEvent(currentRoom, 'MATCH_STATS_REVEAL', packet.payload, currentPlayer.playerID);
        broadcast(currentRoom, packet, ws);
        break;
      ...
      case 'GAME_START':
        ...
        // ── 強制開始處理：沒選陣營或沒選兵種的人轉為觀察者；
        //    有選好但沒按準備的人，把預覽編制當成正式編制 ──
        const forcedSpectators = [];
        for(const p of currentRoom.players.values()){
          if(p.role === 'SPECTATOR') continue;
          const preview = p.loadout || p.previewLoad;
          const hasLoadout = preview && Object.values(preview).some(n => n > 0);
          if(!p.team || !hasLoadout){
            p.role = 'SPECTATOR';
            forcedSpectators.push(p.playerID);
          } else if(!p.loadout){
            p.loadout = p.previewLoad;
          }
        }
        ...
        console.log(`[Start] 房主啟動遊戲，房間 ${roomID}，種子=${currentRoom.mapConfig?.seed}，紅軍先手`);
        {
          const gameStartPayload = {
            spectatorPIDs: forcedSpectators,
      ...
      case 'UNIT_SPAWN':
        ...
        if(packet.payload?.uid){
          ...
          // 安全檢查：uid 會被直接當成 currentRoom.units 的物件 key，
          // 不能是 '__proto__' 等危險字串（見 isSafeUnitID 說明），
          // 也統一要求是非空字串，避免 [object Object] 這類意外轉型。
          if(!isSafeUnitID(uid)){
            console.warn(`[Security] 玩家${currentPlayer.playerID} 送出不合法的 uid（${JSON.stringify(uid)}），已忽略此 UNIT_SPAWN 封包`);
            break;
          }
          ...
          // 斷線重連判斷「是否已部署」用：伺服器只要收過這名玩家的任何 UNIT_SPAWN，就視為他已經部署完成。
          // （PLAYER_DEPLOYED 要等部署後約 5 秒才會送出，這段空窗期斷線的人會被誤判成沒部署過，
          //  重連後又跑一次出生點選擇、造成單位重複。這個旗標只用在 JOIN_ACK.alreadyDeployed，
          //  不會影響 ALL_DEPLOYED 的全員到齊判斷。）
          if(!currentRoom.spawnedPlayers) currentRoom.spawnedPlayers = {};
          currentRoom.spawnedPlayers[currentPlayer.playerID] = true;
      ...
      case 'UNIT_MOVE':
        ...
        // 更新房間單位位置快取
        // 同 UNIT_SPAWN：uid 是直接拿來當 room.units 的物件 key，
        // 必須先過 isSafeUnitID() 這關，否則 '__proto__' 之類的值
        // 會污染整個 Object.prototype（見 isSafeUnitID 說明）。
        if(currentRoom.units && isSafeUnitID(packet.payload?.uid)){
      ...
      case 'REQUEST_UNITS': {
        ...
        //
        // ── 斷線重連（includeOwn=true）──────────────────────────────
        // 平常只推「別人的」單位（自己的單位本來就在自己畫面上）。但重連/重載頁面後，
        // 自己的單位也不在本地了，如果伺服器繼續濾掉，玩家就沒有任何己方單位 →
        // 視野矩陣是空的 → 整張地圖被迷霧蓋成全黑。所以重連時前端會帶
        // includeOwn:true，這裡連自己的單位一起推，並在最後補一則 UNITS_SYNC_DONE
        // （附上這次推送的完整 uid 清單），讓前端知道「補課完成」，
        // 也能據此清掉斷線期間已經陣亡、本地卻還殘留的幽靈單位。
        const includeOwn = !!packet.payload?.includeOwn;
        const myPID = currentPlayer.playerID;
        let pushCount = 0;
        const sentUIDs = [];
        if(currentRoom.units){
          for(const uid of Object.keys(currentRoom.units)){
            const u = currentRoom.units[uid];
            if(includeOwn || u.ownerPID !== myPID){
              _enqueue(ws, { type: 'UNIT_SPAWN', roomID, payload: u });
              pushCount++;
              if(includeOwn) sentUIDs.push(uid);
            }
          }
        }
        if(includeOwn){
          _enqueue(ws, { type: 'UNITS_SYNC_DONE', roomID, payload: { uids: sentUIDs } });
        }
        console.log(`[Units] 推送 ${pushCount} 個單位給玩家${myPID}（批次佇列${includeOwn ? '，含自己單位/重連補課' : ''}）`);
        break;
      }
      ...
      //    這裡只是單純把最終狀態轉發出去。事件本身+座標屬於我方私有資訊，
      //    bug 修正：原本誤用 broadcast() 送給整房（含敵隊），跟前端註解講的
      //    「敵方收不到」不一致，改成 broadcastToTeam 才是原本設計的行為 ──
