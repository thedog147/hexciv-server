...
// ⏱️ 回合轉換保護窗（ms）── 換陣營那一刻，全房間玩家會幾乎同時送出
//    TURN_UPKEEP_SYNC 結算封包（見 advanceToNextTeam），這裡讓「即時操作類」
//    封包延後這麼多 ms 再處理，避開跟結算封包擠在同一瞬間，降低伺服器瞬間負載。
//    純技術性緩衝，不是 UX 停頓，300~800ms 這個區間內都合理，
//    數字越大越保守（CPU 壓力更平緩），但也會讓玩家覺得操作稍微「頓」一下，
//    500ms 是折衷值，可依實測 CPU 曲線調整。
const TURN_TRANSITION_MS = 500;

// 換陣營時，哪些封包類型算「即時操作」要被保護窗擋一下延後處理。
// 只挑會改變戰場狀態、且玩家可能手速很快搶第一時間送出的操作，
// TURN_UPKEEP_SYNC / KILL_TALLY_SYNC / INTEL_SYNC 這類同步/查詢類不受影響。
const GATED_ACTION_TYPES = new Set([
  'UNIT_MOVE', 'ATTACK_RESULT', 'RANGED_ATTACK_RESULT',
  'PSY_SHELL_RESULT', 'PSYOPS_BROADCAST_RESULT',
  'MINE_PLACE', 'MINE_DEFUSE',
]);

// ⏱️ 部署同步逾時（秒）── GAME_START 後，等待全員送出 PLAYER_DEPLOYED 的最長時間。
//    超過這個時間還有人卡在部署畫面（沒選好出生點/掉線），伺服器會強制解鎖，
//    讓其他人不會被卡住的人永遠卡住無法開始。
const DEPLOY_TIMEOUT_SECONDS = 90;
...
// 📦 EXPORT_REPLAY 分批傳送：每批最多幾筆事件。300 筆大約對應幾百 KB 等級的
//    JSON 字串，單次 stringify 成本很小；批次間用 setImmediate 讓出事件迴圈，
//    避免像原本一次 stringify 整包（可能到 3000 筆上限）卡住其他玩家的封包處理。
const EXPORT_REPLAY_CHUNK_SIZE = 300;
...
// maxPayload：單筆訊息大小上限（bytes）。正常操作封包（移動/攻擊/回合結算等）
// 通常遠小於這個值；設這個上限是為了擋掉「client 端 bug 一次塞爆量資料進單一封包」
// 的狀況——這種情況不會被下面的 token bucket 擋到（bucket 只算次數、不算大小）。
// 超過上限時 ws 套件會自動以 code 1009（Message Too Big）關閉該連線，不需要額外處理。
const MAX_INCOMING_PAYLOAD_BYTES = 256 * 1024; // 256KB
const wss = new WebSocketServer({ port: PORT, maxPayload: MAX_INCOMING_PAYLOAD_BYTES });
...
// BATCH_INTERVAL_MS：100ms → 150ms（v104b，因應房間人數擴大到 40 人以上調整）。
// flush 次數（=send()呼叫次數）直接砍 33%，對回合制戰棋（單回合限時 120 秒）
// 而言 0.15 秒的延遲感幾乎無感；若之後實測 CPU 仍吃緊，可再往 200ms 加碼，
// 但不建議超過 250~300ms——空投/PSYOPS廣播/雷達掃描這類跨玩家同步特效
// 會開始有明顯延遲感，邊際 CPU 效益卻已經不大（重複序列化的大頭已由
// 上面的 _packetStrCache 解決，interval 本身空轉幾乎不耗資源）。
...
// ── 封包序列化快取（v104 效能優化，因應房間人數擴大到 40 人以上）──────
// 問題：broadcast()/broadcastToTeam() 會把「同一個」封包物件塞進多個連線
// 各自的佇列裡，但原本每個連線 flush 時都各自重新 JSON.stringify 一次。
// 40 人房間、雙方各 20 人時，一筆全域廣播（例如單位移動、攻擊結果）
// 最多會被重複序列化 39 次，內容完全一樣，純粹浪費 CPU。
// 改成：用 WeakMap 記住「這個封包物件」序列化過的字串，同一個封包物件
// 不管要送給幾個人，JSON.stringify 只跑一次，其餘收件人直接複用字串。
// 用 WeakMap 而非一般 Map，是為了讓封包物件在這一輪 flush 結束、
// 不再被任何佇列引用後可以被 GC 自動回收，不會造成記憶體洩漏。
const _packetStrCache = new WeakMap();
function _stringifyPacket(packet){
  let s = _packetStrCache.get(packet);
  if(s === undefined){
    s = JSON.stringify(packet);
    _packetStrCache.set(packet, s);
  }
  return s;
}
  ...
  for(const [ws, packets] of _pendingBatches){
    ...
    try {
      if(packets.length && ws.readyState === 1){
        // 只有一筆時直接送原始封包（走快取），省掉客戶端多拆一層 BATCH 的開銷；
        // 多筆時用已快取好的單筆字串手動拼接陣列，避免對整包再跑一次 JSON.stringify
        // （payload 裡常包含完整單位/座標資料，重新序列化的成本不小）。
        const out = packets.length === 1
          ? _stringifyPacket(packets[0])
          : '{"type":"BATCH","payload":{"events":[' + packets.map(_stringifyPacket).join(',') + ']}}';
        ws.send(out);
      }
    } catch(err) {
...
// ── 進站佇列：跟上面的出站 BATCH 對稱，把「收到」的封包也攤平處理 ──
// 問題：出站已經用 150ms flush 把「送出」的封包攤平了，但「收到」封包
// 目前是連線一收到就馬上同步處理完。如果短時間內大量封包一起湧入
// （例如部署階段一次生一堆單位、或某個 client 因異常狂發封包），
// Node 事件迴圈會被這些同步處理卡住，害其他玩家的心跳/廣播跟著延遲，
// 在 Render 免費版 CPU 額度很小的情況下這個延遲感會更明顯。
// 解法：連線收到封包時先丟進全域佇列，不馬上處理；用一個固定間隔的
// timer 每次只從佇列取一小批（INCOMING_BATCH_SIZE 筆）出來處理，
// 把 CPU 負擔攤平到多個 tick，而不是在單一瞬間全部處理完。
// 佇列本身仍保持先進先出，事件處理順序跟原本逐筆即時處理完全一致，
// 只是「什麼時候處理」被攤平了，不影響遊戲邏輯正確性。
const INCOMING_TICK_MS    = 20;  // 佇列 drain 間隔（ms）
const INCOMING_BATCH_SIZE = 25;  // 每次 tick 最多處理幾筆（40人房間同時湧入也綽綽有餘）
const _incomingQueue = []; // { ws, raw } 陣列，依到達順序排列

setInterval(() => {
  if(_incomingQueue.length === 0) return;
  const batch = _incomingQueue.splice(0, INCOMING_BATCH_SIZE);
  for(const { ws, raw } of batch){
    if(ws.readyState !== 1) continue; // 連線已關閉/斷線，跳過即可
    try {
      ws._processIncoming(raw);
    } catch (err) {
      // ⚠ 保險網：任何一個封包處理時沒被預期到的例外，都只記錄、不讓整個
      //    Node 程序當掉——避免一個人的怪封包害全房間的人一起斷線。
      console.error('[Server] 處理封包時發生未預期例外（已忽略，伺服器繼續運作）:', err);
    }
  }
}, INCOMING_TICK_MS);

// ── 單一連線流量限制（token bucket）─────────────────────
// 目的：擋掉單一連線短時間內狂發封包（bug 造成的迴圈、或惡意灌爆），
// 避免這種異常流量把整條進站佇列塞爆、拖累同房間其他正常玩家。
// 正常玩家的操作頻率（點擊移動/攻擊/翻頁）距離這個上限還很遠，
// 不會誤傷正常對局。
const INCOMING_RATE_BURST   = 20; // token bucket 容量：允許瞬間連續發這麼多筆
const INCOMING_RATE_PER_SEC = 15; // 每秒回補幾個 token
...
setInterval(() => {
  ...
  console.log(`[Memory] heapUsed=${(mem.heapUsed/1048576).toFixed(1)}MB, rss=${(mem.rss/1048576).toFixed(1)}MB, pendingBatches=${_pendingBatches.size}個連線/共${pendingTotal}筆待送, incomingQueue=${_incomingQueue.length}筆待處理, rooms=${rooms.size}`);
}, 30000);
...
// ── 部署同步逾時計時器（獨立於回合計時器，只在「等待全員部署」階段跑）──
function clearDeployTimer(room){
  if(room.deployTimerHandle){
    clearTimeout(room.deployTimerHandle);
    room.deployTimerHandle = null;
  }
}

// ── 檢查是否全員已部署完成，若是（或 forced=true 逾時強制）就解鎖：
//    廣播 ALL_DEPLOYED，並且從這一刻才正式開始第一回合的計時——
//    這樣「選出生點/等隊友部署」花的時間就不會吃掉玩家的行動時間 ──
function _checkAllDeployedAndUnlock(room, roomID, forced){
  if(!room || room.allDeployedUnlocked) return; // 已經解鎖過，不重複廣播

  const totalCount = Array.from(room.players.values())
    .filter(p => p.role !== 'SPECTATOR').length;
  const deployedCount = Object.keys(room.deployedPlayers || {}).length;

  if(totalCount <= 0) return; // 房間目前沒有玩家（理論上不會發生在這個時機點）
  if(!forced && deployedCount < totalCount) return; // 還有人沒部署完，且尚未逾時

  room.allDeployedUnlocked = true;
  clearDeployTimer(room);

  // 從這一刻才啟動第一回合的限時計時器
  room.activeTeam = room.activeTeam || 'RED';
  room.turnCount  = room.turnCount  || 1;
  startTurnTimer(room, roomID);

  console.log(forced
    ? `[Deploy] 房間 ${roomID} 部署逾時（${deployedCount}/${totalCount}），強制解鎖開始遊戲`
    : `[Deploy] 房間 ${roomID} 全員部署完成（${deployedCount}/${totalCount}），開始遊戲`);

  logReplayEvent(room, 'ALL_DEPLOYED', { deployedCount, totalCount, forced: !!forced }, null);
...
function advanceToNextTeam(room, roomID, forced){
  ...
  // 開啟這次換陣營的保護窗：接下來 TURN_TRANSITION_MS 內，
  // GATED_ACTION_TYPES 裡的即時操作類封包會被延後處理（見 _handleMessage）
  room.turnTransitionUntil = Date.now() + TURN_TRANSITION_MS;
...
wss.on('connection', function (ws) {
  ...
  // ── 流量限制狀態（每個連線各自獨立的 token bucket）──────
  let _msgTokens      = INCOMING_RATE_BURST;
  let _lastRefillTime = Date.now();
  let _rateLimitWarned = false; // 避免被擋期間持續洗一堆警告訊息/log

  function _tryConsumeToken(){
    const now = Date.now();
    const elapsedSec = (now - _lastRefillTime) / 1000;
    if(elapsedSec > 0){
      _msgTokens = Math.min(INCOMING_RATE_BURST, _msgTokens + elapsedSec * INCOMING_RATE_PER_SEC);
      _lastRefillTime = now;
    }
    if(_msgTokens >= 1){
      _msgTokens -= 1;
  ...
  // ── 收到封包：先過流量限制，通過才丟進全域進站佇列（見上方 drain 迴圈），
  //    不在這裡馬上同步處理，避免瞬間爆量卡住整條事件迴圈 ──────────
  ws.on('message', function (raw) {
    if(!_tryConsumeToken()){
      if(!_rateLimitWarned){
        _rateLimitWarned = true;
        console.warn(`[RateLimit] 連線傳送頻率過高，開始丟棄封包（房間：${currentRoom ? getRoomID(currentRoom) : '未加入'}）`);
        try {
          ws.send(JSON.stringify({ type: 'ERROR', payload: { msg: '操作太頻繁，部分指令被忽略，請稍微放慢' } }));
        } catch(e){ /* 送警告失敗也無妨，忽略即可 */ }
      }
      return; // 超過額度直接丟棄，不進佇列
    }
    _incomingQueue.push({ ws, raw });
  });
  ...
  function _handleMessage(raw){
    ...
    // ── 回合轉換保護窗：換陣營那一刻，隊友們的 TURN_UPKEEP_SYNC 結算封包
    //    會在幾毫秒內湧入，如果新行動陣營的玩家手速夠快，第一個操作封包
    //    可能跟這波結算封包擠在同一時間處理。這裡讓「即時操作類」封包
    //    在保護窗（TURN_TRANSITION_MS）內先不處理，用 setTimeout 延後到
    //    窗口結束後再重跑一次同一個 _handleMessage——不是丟棄，只是晚一點
    //    處理，玩家操作不會遺失，體感上幾百毫秒也感覺不出來。
    //    只針對會改變戰場狀態的操作類封包做延後，TURN_UPKEEP_SYNC 這類
    //    結算/同步封包不受影響，該過就過，避免保護窗互相卡住。
    if(GATED_ACTION_TYPES.has(type) && currentRoom && currentRoom.turnTransitionUntil){
      const remainMs = currentRoom.turnTransitionUntil - Date.now();
      if(remainMs > 0){
        setTimeout(() => {
          try { _handleMessage(raw); }
          catch (err) { console.error('[Server] 保護窗延後處理時發生未預期例外（已忽略）:', err); }
        }, remainMs + 10);
        return;
      }
    }
    ...
    if (type === 'JOIN') {
      ...
      ws.send(JSON.stringify({
        ...
        payload: {
          ...
          // ── 晚加入狀態快照：GAME_START/ALL_DEPLOYED 只在觸發當下廣播一次，
          //    晚加入的人（尤其是觀察者）永遠收不到這兩個封包，只能靠這裡
          //    附帶的目前狀態，讓前端知道「遊戲其實已經在進行中」，
          //    不要傻傻卡在大廳等一個不會再來的事件 ──
          gameStarted:         !!room.gameStarted,
          allDeployedUnlocked: !!room.allDeployedUnlocked,
          activeTeam:          room.activeTeam || null,
          turnCount:           room.turnCount || null,
          turnDeadline:        room.allDeployedUnlocked ? (room.turnDeadline || null) : null,
          limitSeconds:        TURN_TIME_LIMIT_SECONDS,
          msg:             isReconnect
    ...
    switch (type) {
     case 'MAP_INIT':
      ...
      case 'PLAYER_DEPLOYED':
        ...
        // 全員部署完成才真正解鎖開始遊戲（見下方 _checkAllDeployedAndUnlock）
        _checkAllDeployedAndUnlock(currentRoom, roomID, false);
        break;
      ...
      case 'GAME_START':
        ...
        // ── IGO-UGO：紅軍永遠先手；但「回合限時計時器」故意不在這裡啟動——
        //    要等全員部署完成（見 _checkAllDeployedAndUnlock）才開始算時間，
        //    避免選出生點、等隊友部署這段時間被算進玩家的行動時限裡 ──
        currentRoom.activeTeam       = 'RED';
        currentRoom.turnCount        = 1;
        currentRoom.endedPlayers     = new Set();
        currentRoom.deployedPlayers  = {};
        currentRoom.allDeployedUnlocked = false;
        clearTurnTimer(currentRoom);
        clearDeployTimer(currentRoom);
        currentRoom.deployTimerHandle = setTimeout(() => {
          _checkAllDeployedAndUnlock(currentRoom, roomID, true); // 逾時強制解鎖
        }, DEPLOY_TIMEOUT_SECONDS * 1000);

        console.log(`[Start] 房主啟動遊戲，房間 ${roomID}，種子=${currentRoom.mapConfig?.seed}，紅軍先手，等待全員部署`);
        {
          const gameStartPayload = {
            ...
            // 注意：這裡刻意不帶 deadline——回合計時器要等 ALL_DEPLOYED 才會啟動
            limitSeconds: TURN_TIME_LIMIT_SECONDS,
            deployTimeoutSeconds: DEPLOY_TIMEOUT_SECONDS,
          };
      ...
      case 'EXPORT_REPLAY': {
        ...
        // ⚠ 分批傳送：對局打很久、事件筆數逼近 MAX_REPLAY_EVENTS 上限時，
        //    單次把整個陣列 JSON.stringify 是同步操作，字串一大就會卡住
        //    事件迴圈一段時間，連帶讓同房間其他人的即時操作跟著延遲。
        //    改成每批只 stringify EXPORT_REPLAY_CHUNK_SIZE 筆，
        //    批次之間用 setImmediate 讓出事件迴圈，插進去的其他玩家封包
        //    可以正常被處理，不會被這次匯出整個擋住。
        const events      = currentRoom.replayEvents || [];
        const turnCount   = currentRoom.turnCount || 1;
        const exportedAt  = Date.now();
        const totalChunks = Math.max(1, Math.ceil(events.length / EXPORT_REPLAY_CHUNK_SIZE));
        const targetRoomID = roomID;
        const targetPID    = currentPlayer.playerID;

        console.log(`[Replay] 玩家${targetPID} 下載房間 ${targetRoomID} 回放紀錄（共 ${events.length} 筆事件，分 ${totalChunks} 批傳送）`);

        let chunkIndex = 0;
        const _sendNextReplayChunk = () => {
          if(ws.readyState !== 1) return; // 連線已斷開，剩餘批次不用再送
          const start = chunkIndex * EXPORT_REPLAY_CHUNK_SIZE;
          const slice = events.slice(start, start + EXPORT_REPLAY_CHUNK_SIZE);
          chunkIndex++;
          try {
            ws.send(JSON.stringify({
              type: 'REPLAY_DATA',
              ...
              payload: {
                ...
                exportedAt,
                events:      slice,
                chunkIndex,   // 1-based，方便前端判斷「這是第幾批」
                totalChunks,  // 前端收滿 totalChunks 批才組裝成完整檔案觸發下載
              }
            }));
          } catch(err){
            console.error(`[Replay] 玩家${targetPID} 分批傳送失敗，中止剩餘批次：`, err.message);
            return;
          }
          if(chunkIndex < totalChunks){
            setImmediate(_sendNextReplayChunk); // 讓事件迴圈喘口氣，處理完排隊中的其他封包再送下一批
          }
        };
        _sendNextReplayChunk();
        break;
      }
      ...
      //    這裡只是單純把最終狀態轉發出去。這份資料含座標＋事件內容，
      //    等同「哪支部隊在哪裡做了什麼」，是該隊的內部資訊，不能讓對方
      //    知道（否則等於幫敵方洩漏我方單位位置與行動）——比照
      //    TURN_UPKEEP_SYNC，只送給同隊，不整房廣播 ──
      ...
      // ── 戰術版情報標記（敵情/補給/地雷）：只送給同隊隊友，紅藍雙方的
      //    戰術版各自獨立，不共用彼此標註的內容（比照 broadcastToTeam
      //    保護後勤機密數據的做法，情報標記同樣不該讓對面看到）──
      case 'INTEL_SYNC':
  ...
  // 讓全域進站佇列的 drain 迴圈可以呼叫到「這條連線」專屬的 _handleMessage
  // （_handleMessage 用到的 currentRoom/currentPlayer 是這個 connection closure 裡的變數，
  //  所以必須維持每條連線各自一份，不能把它改成全域共用函式）
  ws._processIncoming = _handleMessage;

  // ── 斷線處理 ───────────────────────────────────────
  ws.on('close', function (code, reason) {
    if(code === 1009){
      console.warn(`[Security] 連線因訊息過大被強制關閉（超過 ${MAX_INCOMING_PAYLOAD_BYTES} bytes 上限），房間：${currentRoom ? getRoomID(currentRoom) : '未加入'}，玩家：${currentPlayer ? (currentPlayer.name || currentPlayer.playerID) : '未知'}`);
    }
    if (currentRoom && currentPlayer) {
      ...
      // 如果遊戲已開始、還在等全員部署的階段，這個人一走，剩下的人數門檻也跟著變低——
      // 重新檢查一次，避免「卡住的那個人」剛好是掉線的人，導致其他人被永久卡住等不到 ALL_DEPLOYED
      if(currentRoom.gameStarted && !currentRoom.allDeployedUnlocked){
        _checkAllDeployedAndUnlock(currentRoom, getRoomID(currentRoom), false);
      }
