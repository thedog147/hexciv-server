...
wss.on('connection', function (ws) {
  ...
  // ── 收到封包 ───────────────────────────────────────
  // ── 防洪水保護（token bucket）：正常操作（點擊、移動）不可能每秒超過
  //    這個量，超過大概率是前端 bug（例如某個事件監聽器沒解綁、重複觸發）
  //    或惡意封包灌爆。超量直接丟棄該筆封包，只記警告，不斷線、不讓
  //    Node 花時間處理，避免單一失控連線拖垮整個房間。
  ...
  ws.on('message', function (raw) {
    ...
    _lastRefill = now;
    if(_msgTokens < 1){
      console.warn(`[RateLimit] 玩家${currentPlayer?.playerID ?? '?'} 封包頻率超限，丟棄本次封包`);
      return;
    }
    _msgTokens -= 1;
    try {
